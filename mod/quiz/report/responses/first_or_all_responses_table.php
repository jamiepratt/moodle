...
class quiz_first_or_all_responses_table extends quiz_last_responses_table {
    ...
    protected function field_from_extra_data($attempt, $slot, $field) {
        ...
        switch($field) {
            ...
            case 'responsesummary' :
                return $this->get_question_attempt($attempt->usageid, $slot)->get_summary_after_try($attempt->try);
            case 'rightanswer' :
    ...
    protected function load_extra_data() {
        ...
        foreach ($this->rawdata as $attempt) {
            $mostriesforallparts = 1;
            foreach ($this->questionusagesbyactivity[$attempt->usageid]->get_slots() as $slot) {
                $tries = $this->get_question_attempt($attempt->usageid, $slot)->get_no_of_tries();
                $mostriesforallparts = max($mostriesforallparts, $tries);
            }
            for ($try = 1; $try <= $mostriesforallparts; $try++) {
                $newattemptrow = clone($attempt);
                $newattemptrow->lasttryforallparts = ($try == $mostriesforallparts);
                if ($try !== $mostriesforallparts) {
    ...
    protected function slot_state($attempt, $slot) {
        return $this->get_question_attempt($attempt->usageid, $slot)->get_state_after_try($attempt->try);
    }
    ...
    protected function slot_fraction($attempt, $slot) {
        return $this->get_question_attempt($attempt->usageid, $slot)->get_fraction_after_try($attempt->try);
    }
     ...
     */
    protected function step_no_for_try($questionusageid, $slot, $try) {
        return $this->get_question_attempt($questionusageid, $slot)->get_step_no_with_try($try);
    }
