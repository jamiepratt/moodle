...
class quiz_statistics_report extends quiz_default_report {
    ...
    public function display($quiz, $cm, $course) {
        ...
        if (!$nostudentsingroup) {
            ...
            list($quizstats, $questionstats, $subquestionstats) =
                $this->get_all_stats_and_analysis($quiz, $whichattempts, $groupstudents, $questions);
        } else {
     ...
     */
    protected function get_all_stats_and_analysis($quiz, $whichattempts, $groupstudents, $questions) {

        $qubaids = quiz_statistics_qubaids_condition($quiz->id, $groupstudents, $whichattempts);

        $qcalc = new \core_question\statistics\questions\calculator($questions, $this->get_progress_trace_instance());

        $quizcalc = new \quiz_statistics\calculator($this->get_progress_trace_instance());
    ...
    protected function analyse_responses_for_all_questions_and_subquestions($qubaids, $questions, $subquestionstats) {
        ...
        $responseprogress->start_progress('', count($questions) + count($subquestionstats));

        // Starting response analysis of main questions.
        $responseprogress->start_progress('', count($questions), count($questions));

        $done = array();
        $donecount = 1;
        foreach ($questions as $question) {
            $responseprogress->progress($donecount);
            $donecount++;
            if (!question_bank::get_qtype($question->qtype, false)->can_analyse_responses()) {
                continue;
            }
        ...
        $responseprogress->end_progress();

        // Starting response analysis of sub-questions.
        $countsubquestions = count($subquestionstats);
        $responseprogress->start_progress('', $countsubquestions, $countsubquestions);
        $donecount = 1;
