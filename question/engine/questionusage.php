...
class question_usage_by_activity {
     ...
     *
     * @param $simulatedresponsedata an array of simulated data, keys are slot nos and values are arrays representing student
     *                                   responses which will be passed to question_definition::transform_response method and then
     *                                   have the appropriate prefix added.
     * @return array simulated post data
     */
    public function get_post_data($simulatedresponsedata) {
        $simulatedpostdata = array();
        $simulatedpostdata['slots'] = implode(',', array_keys($simulatedresponsedata));
        foreach ($simulatedresponsedata as $slot => $responsedata) {
            $prefix = $this->get_field_prefix($slot);
            $slotresponse = $this->get_question($slot)->transform_response($responsedata);
